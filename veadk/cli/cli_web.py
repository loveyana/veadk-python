...
def _patch_adkwebserver_oauth2(
    ...
    Args:
        ...
        redirect_uri: OAuth2 redirect URI (e.g., http://localhost:8000/oauth2/callback).
    """
    ...
    def wrapped_get_fast_api(self, *args, **kwargs):
        ...
        # Determine cookie_secure based on redirect_uri
        # Disable cookie_secure for localhost/127.0.0.1 development
        is_local = redirect_uri.startswith("http://localhost") or redirect_uri.startswith("http://127.0.0.1")
        ...
        oauth2_config.cookie_secure = cookie_secure

        # ADK web server static/frontend paths that should skip OAuth2
        adk_exempt_prefixes = [
            "/dev-ui/",  # ADK dev UI static files
            "/static/",  # Static files
        ...
        setup_oauth2(
            app,
            oauth2_config,
            exempt_prefixes=adk_exempt_prefixes,
        )
